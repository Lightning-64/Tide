...
public class TideRecipeProvider extends FabricRecipeProvider {
    ...
    private void createSimpleBobberRecipe(Consumer<FinishedRecipe> output, Item bobber, Item addition) {
    ...
    private void createColoredBobberRecipes(Consumer<FinishedRecipe> output) {
        TagKey<Item> slimeballTag = conventionTag("slime_balls");
        BOBBERS.forEach(bobber -> {
            String dyeId = bobber.toString().split("_fishing_bobber")[0];
            TagKey<Item> dyeTag = TagKey.create(Registries.ITEM, new ResourceLocation(
                    "c", "dyes/" + dyeId));
